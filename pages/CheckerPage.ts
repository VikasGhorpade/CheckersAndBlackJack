...
export class CheckerPage {
  ...
  async getBoard() {
    ...
    await listOfRows.last().waitFor({ state: 'visible', timeout: 50000 });
    let map = new Map();
  ...
  async makeMove(selectorFrom, selectorTo) {
    ...
    await this.page.getByText("Make a move.").waitFor({ state: 'visible' });
    // await this.page.waitForTimeout(10000);
  }
  async calculateNextMove(map: Map<string, string>, round: number) {
    ...
    return new Promise(async (resolve) => {
      ...
      if (captureMove.size > 0) {
        ...
        resolve(await this.getPreferredMove(captureMove));
      } else {
        // let nextMoveValue = await this.getPreferredMove(simpleMoves);
        // return nextMoveValue;
        resolve(await this.getPreferredMove(simpleMoves));
      }
    });
  }
  ...
  async getPreferredMove(preferredMove: Map<string, string[]>) {
    ...
    let promise = new Promise((resolve) => {
      timeoutId = setTimeout(() => {
        ...
        console.log("space" + positionX + positionY + " My move: " + value);
        resolve(["space" + positionX + positionY, value ? value[0] : null]);
      }, 5000);
    });
